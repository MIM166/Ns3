...
class TcpSocketBase : public TcpSocket
{
  public:
    ...
    /**
     * @brief Get the congestion control algorithm object.
     *
     * Allows simulation scratch files (e.g., Figure-10 reproduction) to
     * obtain the TcpCongestionOps instance without modifying the application
     * layer.  In particular, used to retrieve a TcpBolt pointer so that
     * ProcessExternalSrcPacket() can call TcpBolt::ProcessSrcPacket().
     *
     * @return Pointer to the active TcpCongestionOps instance.
     */
    Ptr<TcpCongestionOps> GetCongestionControlAlgorithm() const;

    /**
     * @brief Deliver a BoltQueue SRC (Back-To-Sender) packet directly to the
     *        sender's congestion-control algorithm.
     *
     * In a real deployment the SRC packet travels through the network and is
     * detected by ReceivedData() via the BTS flag.  In ns-3 scratch simulations
     * the SRC callback on BoltQueue can instead call this method after a
     * simulated propagation delay, fully reproducing the sub-RTT feedback loop
     * without needing a reverse TCP connection from the switch.
     *
     * Only acts if the congestion control is TcpBolt; silently returns otherwise.
     *
     * @param srcPacket  Packet carrying a BoltHeader with BTS|DECWIN flags
     *                   (as generated by BoltQueue::GenerateSrcPacket).
     */
    void ProcessExternalSrcPacket(Ptr<Packet> srcPacket);

    /**
     * @brief Deliver external Bolt SRC feedback using an explicit link rate.
     *
     * WIRELESS ADAPTATION:
     * The wired BoltQueue path encodes only a small fixed set of link-rate
     * flags inside BoltHeader.  Static 802.15.4 uses a much lower physical
     * rate, so scratch-level adapters need a way to preserve the Bolt control
     * law while supplying the true link rate directly.
     *
     * This is plumbing only.  TcpBolt::ProcessSrcPacket() is unchanged.
     *
     * @param srcPacket  Packet carrying a BoltHeader with BTS|DECWIN flags
     * @param linkRate   Actual bottleneck link rate seen by the external adapter
     */
    void ProcessExternalSrcPacket(Ptr<Packet> srcPacket, DataRate linkRate);
    ...
    /**
     * Record the current application-data tail as the point where BOLT should
     * start treating packets as part of the final window of the flow.
     */
    void PrepareBoltFlowClosure();

    /**
     * Tell TCP the total number of application bytes this flow will send so
     * the BOLT LAST window can be marked before completion.
     */
    void SetBoltExpectedFlowBytes(uint64_t totalBytes);
  ...
  protected:
    ...
    SequenceNumber32 m_boltFinalTailSequence{0}; //!< Final app-data tail for LAST marking
    bool m_hasBoltFinalTailSequence{false};      //!< Whether final tail has been recorded
    ...
    BoltHeader m_lastReceivedBoltHeader;
    BoltHeader m_currentSegmentBoltHeader;
    bool m_currentSegmentHasBoltHeader{false};
    bool m_currentSegmentAckHasInc{false};
};
